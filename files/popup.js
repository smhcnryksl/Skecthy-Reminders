document.addEventListener('DOMContentLoaded', async () => {
    ...
    function updateLocalization() {
        ...
        const monthSelect = document.getElementById('yearly-month-select');
        if (monthSelect) {
            const monthNames = getT('months');
    ...
    async function updateCountdowns() {
        ...
        for (const [key, el] of Object.entries(timers)) {
            ...
            if (alarm) {
                ...
                if (key === 'alarm_prayer') {
                    const pNameRaw = alarm.name.replace('prayer_instance_', '');
                    const pNameLocal = getT(`prayer_${pNameRaw.toLowerCase()}`) || pNameRaw;
                    el.textContent = diff > 0 ? `${pNameLocal}: ${formatTime(diff)}` : "Triggering...";
                } else {
